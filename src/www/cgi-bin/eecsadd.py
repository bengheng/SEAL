...
mailverify = form.getvalue("mailverify", None)
...
def mailverify_is_valid(vcode):
	uniqname = None
	cur = conn.cursor()

	try:
		cur.execute('SELECT uniqname FROM verification WHERE veristring=?', (vcode,))
		...
		cur.execute('DELETE FROM verification WHERE veristring=?', (vcode,))
		conn.commit()
	...
	if cur.rowcount < 1 or not uniqname:
		return False

	return uniqname

def add_user( conn, usrname, pwd, fwdaddr, salt, retrievalkey, uniqname ):
	cur = conn.cursor()
	...
	try:
		cur.execute( "INSERT OR FAIL INTO `user` " \
                                ...
                                "(?, ?, ?, ?, ?, ?);",
                                (usrname, pwd, fwdaddr, salt, retrievalkey, uniqname) )
		conn.commit()
...
uniqname = mailverify_is_valid(mailverify)
if err == '' and not uniqname:
	err += '<li>Bad email verification.</li>\n'
...
if err == '':
	...
	add_user( conn, accountname, hashedpwd, fwdaddx, salt, retrievalkey, uniqname )
else:
