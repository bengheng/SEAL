...
class dockexample2(com.hp.hpl.guess.ui.DockableAdapter):
	...
	def __init__(self):
		...
		self.testSlider.setMinimum(timestamp.min)
		self.testSlider.setMaximum(timestamp.max + 1)
		...
		self.testSlider.setValue(timestamp.min)  # default value
	...
	def sc(self,evt):
		...
		#for e in g.edges:
		#	(timestamp > val).visible = 0

		(timestamp <= val).visible = 1
		(timestamp > val).visible = 0
		...
		# set the label text
		self.label.setText("Timestamp threshold ("+str(val)+")")
