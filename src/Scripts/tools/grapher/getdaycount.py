...
for i in range(0,35):
	...
	while line:
		...
		numForDay = getCountOnDay( db, line, curdate)
		totalForDay = totalForDay + numForDay

		n = n + 1
		line = arfile.readline().rstrip()

	arfile.close()
	#sys.stdout.write( str(totalForDay) + '\n' )
	totalForDays.append(totalForDay)

maxForDay = 0
for i in range(0,35):
	if totalForDays[i] > maxForDay:
		maxForDay = totalForDays[i]

for i in range(0, 35):
	print str(i+1) + ' ' + str( float(totalForDays[i]) / float(maxForDay) )
